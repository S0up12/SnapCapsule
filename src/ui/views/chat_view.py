...
class ChatBubble(ctk.CTkFrame):
    ...
    def render_media_placeholder(self, parent, path):
        btn = ctk.CTkButton(parent, text="...", width=200, height=150, 
    ...
    def _apply_error_main_thread(self, btn, is_video, path):
        if not btn.winfo_exists(): return
        # UPDATED: Use Icons for error states
        if is_video:
            icon = assets.load_icon("play", size=(24, 24))
            btn.configure(text=" Play (No Preview)", image=icon, compound="left", state="normal", 
                          command=lambda: self.media_callback(path))
        else:
...
class ChatView(ctk.CTkFrame):
    def __init__(self, parent, chats_data, profile_data=None):
        super().__init__(parent, fg_color="transparent")
    ...
    def _setup_ui(self):
        ...
        # UPDATED: Custom Search Bar with Icon
        ...
        self.populate_friends(sorted(self.chats.keys()))
    ...
    # --- SHOW MEDIA WITH CAROUSEL ---
    def show_media(self, path):
        if not os.path.exists(path): return
        
        # 1. Collect all media in this chat for the playlist
        playlist = []
        all_msgs = self.chats.get(self.current_friend_key, [])
        for msg in all_msgs:
            if msg['media']:
                for p in msg['media']:
                    playlist.append(p)
        ...
        try:
            ...
            idx = 0
            playlist = [path] # Fallback
            
        # 3. Launch Global Player
        GlobalMediaPlayer(self, playlist, idx)

    def update_search(self, event=None):
        q = self.search_entry.get().lower()
        lst = [n for n in self.chats.keys() if q in n.lower()] if q else sorted(self.chats.keys())
        self.populate_friends(lst)
    ...
    def load_chat(self, chat_key):
        ...
        all_msgs = self.chats[chat_key]
        self.total_msgs = len(all_msgs)
    ...
    def render_window(self, scroll_pos=1.0):
        ...
        msgs = self.chats[self.current_friend_key][self.view_start : self.view_end]
        
        if self.view_start > 0:
            # UPDATED: Use Arrow Icon
        ...
        for i, msg in enumerate(msgs):
            ...
            if d != last_date:
                ...
                ctk.CTkLabel(f, text=datetime.strptime(d, "%Y-%m-%d").strftime("%B %d").upper(), 
